...
class GameScene: SKScene {
    ...
    override func didMove(to view: SKView) {
        ...
        counterClockwiseButton.position = CGPoint(x: view.scene!.frame.minX+240, y: view.scene!.frame.minY+30)
        //цвет заливки
        ...
        clockwiseButton.position = CGPoint(x: view.scene!.frame.maxX-90, y: view.scene!.frame.minY+30)
        ...
        //задаем форму круга
        resetButton.path = UIBezierPath(ovalIn: CGRect(x: 0, y: 0, width: 60, height: 60)).cgPath
        //указываем координаты размещения
        resetButton.position = CGPoint(x: view.scene!.frame.minX+120, y: view.scene!.frame.minY+30)
        //цвет заливки
        ...
        snake = Snake(atPoint: CGPoint(x: view.scene!.frame.midX, y: view.scene!.frame.midY))  //также будет если врезаться в стену
        self.addChild(snake!)
    }
    ...
    override func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        for touch in touches {
            ...
            guard let touchedNode = self.atPoint(touchLocation) as? SKShapeNode,
                touchedNode.name == "counterClockwiseButton" || touchedNode.name == "clockwiseButton" || touchedNode.name == "resetButton" else {
                    return
            }
            ...
            if touchedNode.name == "counterClockwiseButton" {
                ...
                snake = Snake(atPoint: CGPoint(x: (view?.scene!.frame.minX)!+200, y: (view?.scene!.frame.minY)!+30)) //создает змею
                self.addChild(snake!)           //добавляет змею
            }
...
extension GameScene: SKPhysicsContactDelegate {
    func didBegin(_ contact: SKPhysicsContact) {
        ...
        case CollisionCategories.EdgeBody: //проверяем что это стенка экрана
            snake?.removeFromParent()       //удаляет змею
